...
public class StockController {
	...
	// ✅ FIXED: Added try-catch to handle RazorpayException
	...
	public String rechargeWallet(@RequestParam double amount, HttpSession session, Model model) {
		try {
			...
			e.printStackTrace();
			session.setAttribute("fail", "Payment error: " + e.getMessage());
			return "redirect:/wallet";
		} catch (Exception e) {
			e.printStackTrace();
			session.setAttribute("fail", "Error: " + e.getMessage());
			return "redirect:/wallet";
		}
	}

	@PostMapping("/payment-success")
	public String paymentSuccess(@RequestParam double amount, HttpSession session) {
